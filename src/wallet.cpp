...
// paccoin: optional setting to unlock wallet for block minting only;
...
bool CWallet::AddToWallet(const CWalletTx& wtxIn)
{
    uint256 hash = wtxIn.GetHash();
    {
        ...
        scriptDefaultKey.SetPaccoinAddress(vchDefaultKey);
        BOOST_FOREACH(const CTxOut& txout, wtx.vout)
        {
            if (txout.scriptPubKey == scriptDefaultKey)
            {
                ...
                if (GetKeyFromPool(newDefaultKey, false))
                {
                    ...
                    SetAddressBookName(CPaccoinAddress(vchDefaultKey), "");
                }
            }
        }
#endif
...
void CWalletTx::GetAmounts(int64& nGeneratedImmature, int64& nGeneratedMature, list<pair<CPaccoinAddress, int64> >& listReceived,
                           list<pair<CPaccoinAddress, int64> >& listSent, int64& nFee, string& strSentAccount) const
{
                                  ...
                                  int64& nSent, int64& nFee) const
{
    ...
    }
    {
        ...
        BOOST_FOREACH(const PAIRTYPE(CPaccoinAddress,int64)& r, listReceived)
        {
            ...
            {
                map<CPaccoinAddress, string>::const_iterator mi = pwallet->mapAddressBook.find(r.first);
...
// paccoin: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // paccoin: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // paccoin: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    if (!GetBoolArg("-avatar")) // paccoin: not avatar mode
                    {
                        ...
                        scriptChange.SetPaccoinAddress(vchPubKey);
                    }
...
// paccoin: create coin stake transaction
...
string CWallet::SendMoneyToPaccoinAddress(const CPaccoinAddress& address, int64 nValue, CWalletTx& wtxNew, bool fAskFee)
{
    // Check amount
    if (nValue <= 0)
...
bool CWallet::SetAddressBookName(const CPaccoinAddress& address, const string& strName)
{
...
bool CWallet::DelAddressBookName(const CPaccoinAddress& address)
{
...
// paccoin: check 'spent' consistency between wallet and txindex
// paccoin: fix wallet spent state according to txindex
...
// paccoin: disable transaction (only for coinstake)
...
void CWallet::GetAllReserveAddresses(set<CPaccoinAddress>& setAddress)
{
    setAddress.clear();
    ...
    BOOST_FOREACH(const int64& id, setKeyPool)
    {
        ...
        CPaccoinAddress address(keypool.vchPubKey);
        assert(!keypool.vchPubKey.empty());
        if (!HaveKey(address))
