...
// paccoin: get network Gh/s estimate
...
Value getnewaddress(const Array& params, bool fHelp)
{
    ...
    CPaccoinAddress address(newKey);
...
CPaccoinAddress GetAccountAddress(string strAccount, bool bForceNew=false)
{
    ...
    if (!account.vchPubKey.empty())
    {
        ...
        scriptPubKey.SetPaccoinAddress(account.vchPubKey);
    ...
    if (account.vchPubKey.empty() || bForceNew || bKeyUsed)
    {
        ...
        pwalletMain->SetAddressBookName(CPaccoinAddress(account.vchPubKey), strAccount);
        walletdb.WriteAccount(strAccount, account);
    }

    return CPaccoinAddress(account.vchPubKey);
}
...
Value setaccount(const Array& params, bool fHelp)
{
    ...
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid paccoin address");
...
Value getaccount(const Array& params, bool fHelp)
{
    ...
    string strAccount;
    map<CPaccoinAddress, string>::iterator mi = pwalletMain->mapAddressBook.find(address);
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    ...
    string strError = pwalletMain->SendMoneyToPaccoinAddress(address, nAmount, wtx);
    if (strError != "")
...
Value verifymessage(const Array& params, bool fHelp)
{
    ...
    return (CPaccoinAddress(key.GetPubKey()) == addr);
}
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    ...
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid paccoin address");
    scriptPubKey.SetPaccoinAddress(address);
...
void GetAccountAddresses(string strAccount, set<CPaccoinAddress>& setAddress)
{
    BOOST_FOREACH(const PAIRTYPE(CPaccoinAddress, string)& item, pwalletMain->mapAddressBook)
    {
        const CPaccoinAddress& address = item.first;
...
Value getbalance(const Array& params, bool fHelp)
{
    ...
    if (params[0].get_str() == "*") {
        ...
        for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
        {
            ...
            list<pair<CPaccoinAddress, int64> > listReceived;
            list<pair<CPaccoinAddress, int64> > listSent;
            wtx.GetAmounts(allGeneratedImmature, allGeneratedMature, listReceived, listSent, allFee, strSentAccount);
            if (wtx.GetDepthInMainChain() >= nMinDepth)
            {
                BOOST_FOREACH(const PAIRTYPE(CPaccoinAddress,int64)& r, listReceived)
                    nBalance += r.second;
            }
            BOOST_FOREACH(const PAIRTYPE(CPaccoinAddress,int64)& r, listSent)
                nBalance -= r.second;
            nBalance -= allFee;
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(-5, string("Invalid paccoin address:")+s.name_);
...
void ListTransactions(const CWalletTx& wtx, const string& strAccount, int nMinDepth, bool fLong, Array& ret)
{
    ...
    {
        BOOST_FOREACH(const PAIRTYPE(CPaccoinAddress, int64)& s, listSent)
        {
            Object entry;
...
Value listaccounts(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const PAIRTYPE(CPaccoinAddress, string)& entry, pwalletMain->mapAddressBook) {
...
Value walletpassphrase(const Array& params, bool fHelp)
{
    ...
    // paccoin: if user OS account compromised prevent trivial sendmoney commands
    if (params.size() > 2)
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; paccoin server stopping, restart to run with encrypted wallet";
}
...
Value validateaddress(const Array& params, bool fHelp)
{
    ...
    if (isValid)
    {
        ...
        else if (pwalletMain->HaveCScript(address.GetHash160()))
        {
            ...
            std::vector<CPaccoinAddress> addresses;
            txnouttype whichType;
            int nRequired;
            ExtractAddresses(subscript, whichType, addresses, nRequired);
            ret.push_back(Pair("script", GetTxnOutputType(whichType)));
            Array a;
            BOOST_FOREACH(const CPaccoinAddress& addr, addresses)
...
Value getblocktemplate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "See https://en.paccoin.it/wiki/BIP_0022 for full specification.");
...
// paccoin: get information of sync-checkpoint
...
// paccoin: reserve balance from being staked for network protection
...
// paccoin: check wallet integrity
...
// paccoin: make a public-private key pair
